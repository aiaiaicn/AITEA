...
- (BOOL)xcodeCLTInstalled {
    ...
    if (error) {
        NSLog(@"aitea: xcodeCLTInstalled [error]: %@", error);
        return NO;
    }
    
    [task waitUntilExit];

    NSLog(@"aitea: xcodeCLTInstalled [output]: %d", task.terminationStatus);
...
- (BOOL)xcodeInstalled {
    ...
    if (error) {
        NSLog(@"aitea: xcodeInstalled [error]: %@", error);
        return NO;
    }
    ...
    NSLog(@"aitea: xcodeInstalled [output]: %@", output);
...
- (void)openTwitterURLWithStars:(int)starCount {
    ...
    NSString *message = [NSString stringWithFormat:@"I got %@ developer security with @teaprotocol’s aitea", stars];
    ...
    NSString *urlString = [NSString stringWithFormat:@"https://twitter.com/intent/tweet?=%@", encodedMessage];
...
- (IBAction)createSSHPrivateKey:(NSSwitch *)sender {
    ...
    if (sender.state == NSControlStateValueOn) {
        NSArray *arguments = @[@"-t", @"ed25519", @"-C", @"Generated by aitea", @"-f", path, @"-N", @""];
        ...
        alert.informativeText = @"Deleting your SSH key pair cannot be undone by aitea.";
...
@implementation aitea (NSTextFieldDelegate)
...
@implementation aitea (Integration)
...
@implementation aitea (dotfileSync)
...
- (BOOL)dotfileDirThere {
    BOOL isdir = NO;
    id path = [NSHomeDirectory() stringByAppendingPathComponent:@"Library/Application Support/aitea/dotfiles.git"];
...
- (IBAction)onDotfileSyncToggled:(NSSwitch *)sender {
    ...
    if (sender.state == NSControlStateValueOn) {
        ...
        NSLog(@"aitea: %@", self.dotfileDirThere ? @"YES" :@"NO");
...
@implementation aitea (NSTableViewDataSource)
