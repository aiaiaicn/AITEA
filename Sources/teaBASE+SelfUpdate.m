...
@implementation aitea (SelfUpdate)
...
- (void)checkForUpdates {
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:rq completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (error) return NSLog(@"aitea: fetch error: %@", error.localizedDescription);
        if (!data) return NSLog(@"aitea: no data from: %@", url);
...
void download(id url, aitea *self) {
#if DEBUG
    NSLog(@"aitea: would update to: %@", url);
#else
